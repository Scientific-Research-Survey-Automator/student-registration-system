...
BEGIN
    IF INSERTING THEN
        INSERT INTO LOGS
        VALUES (logseq.NEXTVAL, (SELECT USER FROM dual), CURRENT_TIMESTAMP, 'G_ENROLLMENTS', 'INSERT', TO_CHAR(:NEW.G_B#) || ',' || TO_CHAR(:NEW.CLASSID));
    ELSIF DELETING THEN
        INSERT INTO LOGS
        VALUES (logseq.NEXTVAL, (SELECT USER FROM dual), CURRENT_TIMESTAMP, 'G_ENROLLMENTS', 'DELETE', TO_CHAR(:OLD.G_B#) || ',' || TO_CHAR(:OLD.CLASSID));
END IF;
END;^
...
-- END;^
--
-- CREATE OR REPLACE TRIGGER enrollment_logging
...
-- ^
--
-- -- Enrolling/De-enrolling Trigger
--
-- CREATE OR REPLACE TRIGGER enrollment_update
-- BEFORE INSERT OR DELETE
-- ON G_ENROLLMENTS
-- FOR EACH ROW
-- BEGIN
...
-- CREATE OR REPLACE TRIGGER enrollment
-- BEFORE
-- INSERT
--     OR
-- DELETE
...
--     CLASSES c
-- SET
--     c.CLASS_SIZE = c.CLASS_SIZE  + 1
-- WHERE
--         c.CLASSID = :NEW.CLASSID;
--
-- ELSIF DELETING THEN
--         INSERT INTO LOGS
--             VALUES
